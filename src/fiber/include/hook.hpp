...
// 告诉 C++ 编译器，这部分代码要用 C 语言的链接规范。
extern "C" {

typedef unsigned int (*sleep_fun)(unsigned int seconds);    // 函数指针
extern sleep_fun sleep_f;   // 声明了一个全局的函数指针:作用：用来保存原始的、未被 Hook 的系统函数地址。
...
extern int connect_with_timeout(int fd, const struct sockaddr *addr, socklen_t addrlen, uint64_t timeout);
}