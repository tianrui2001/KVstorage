...
//   // 【新增：向百度发送一个简单的 HTTP GET 请求】, 它会给我返回一个 HTML 网页，此时就能看到 read 事件被执行了
...
void watch_io_read() {
    std::cout << "watch_io_read begin" << std::endl;
    monsoon::IOManager::getThis()->addEvent(sockfd, monsoon::Event::READ, do_io_read);
}

void test_io() {
    sockfd = socket(AF_INET, SOCK_STREAM, 0);
    monsoon::CondPanic(sockfd > 0, "socket error");
  ...
  if (rt != 0) {
    if (errno == EINPROGRESS) {
      ...
      monsoon::IOManager::getThis()->addEvent(sockfd, monsoon::Event::WRITE, do_io_write);
      // 注册读事件回调，注意事件是一次性的
      monsoon::IOManager::getThis()->addEvent(sockfd, monsoon::Event::READ, do_io_read);
    } else {
